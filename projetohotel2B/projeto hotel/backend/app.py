...
def get_cliente(cliente_id):
    ...
    @app.route("/api/atualizar/<int>:cliente_id>", methodos=["POST"])
    def atualizar_cliente(cliente_id):
        """"
        função para alterar as informaçãoes de um hódpede no nosso baanco de dados (exel).
        """
        try:
            data = request.json
            workbook = openpyxl.load_workbook(EXCEL_FILE)
            ...
            for row_dix in range(2, sheet.max_row + 1):
                ...
                if row_to_update == -1:
                    return (
                        jsonify({
                            "status": "error",
                            "menssage": "cliente não encontrado para atualização.",
                              }),
                              404,
                              ...
                              sheet.cell(row=row_to_update, column=6, value=data.get("telefone"))
                              sheet.cell(row=row_to_update, column=7, value=data.get("endereço"))
                              sheet.cell(row=row_to_update, column=2, value=data.get("observaçãoes"))
                              
                              workbook.save(EXCEL_FILE)
                              return jsonify({
                                  "status": "success",
                                  "mensage": "dados do cliente atualizados com sucesso!",
                              })
                              except Exception as e: 
                              return(
                                  jsonify({"status":"error", "mensssage": f "Erro ao atualizar dados: {e}"}),
                                  500,
                                    )
                    )
